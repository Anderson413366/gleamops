...
import { MapPin, Users, Pencil, Building2, Key, Briefcase, Package, Shield } from 'lucide-react';
...
export function SiteDetail({ site, open, onClose, onEdit }: SiteDetailProps) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      supabase.from('site_supplies').select('id, supply_id, par_level, supply:supply_catalog!site_supplies_supply_id_fkey(name, category)').eq('site_id', site.id).is('archived_at', null),
