...
export default function FinancialIntelligenceClient() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const avgInspectionScorePct = completedInspections.length > 0
      ? Math.round((completedInspections.reduce((s, row) => s + (row.score_pct ?? 0), 0) / completedInspections.length) * 10) / 10
      : 0;
        ...
        <ChartCard title="Risk Signals" subtitle="Concentration and proxy health indicators">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              <p className="text-xs text-muted-foreground">Warns if a single client represents 25%+ of active contract revenue (monthly est.).</p>
            </div>
