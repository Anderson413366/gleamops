...
export default function HomePage() {
  ...
  const fetchDashboard = useCallback(async () => {
    ...
    const [auditRes, ticketListRes, prospectRes, shiftsRes, lowStockRes, inventoryDueRes, clientIssuesRes, expiringCertsRes, expiringTrainingRes, expiringDocsRes] = await Promise.all([
      ...
      supabase
        .from('time_entries')
        .select('id, staff_id, start_at, staff:staff!time_entries_staff_id_fkey(full_name)')
        .is('end_at', null)
      ...
      supabase
        .from('site_supplies')
        .select('id, name, category, par_level, supply_id, site_id, site:sites!site_supplies_site_id_fkey(name)')
        .is('archived_at', null)
        .gt('par_level', 0)
        .order('name')
        .limit(200),
      ...
      supabase
        ...
        .select('id, certification_name, expiry_date, staff:staff!staff_certifications_staff_id_fkey(full_name)')
        .is('archived_at', null)
        .eq('status', 'ACTIVE')
