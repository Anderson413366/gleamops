  ...
  useEffect(() => {
    ...
    async function openTicketFromQuery(ticketId: string) {
      ...
      const { data, error } = await supabase
        .from('work_tickets')
        .select(
          `*, job:job_id!work_tickets_job_id_fkey(job_code, billing_amount), site:site_id!site_jobs_site_id_fkey(site_code, name, address, client:client_id!sites_client_id_fkey(name, client_code))`,
        )
        .eq('id', ticketId)
        .is('archived_at', null)
        .single();
