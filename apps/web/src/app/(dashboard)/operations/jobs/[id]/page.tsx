...
export default function JobDetailPage() {
  ...
  const fetchAssignments = async (jobId: string) => {
    ...
    const { data } = await supabase
      ...
      .select(`
        ...
        start_date,
        end_date,
        staff:staff!job_staff_assignments_staff_id_fkey(id, staff_code, full_name, staff_status)
      `)
      .eq('job_id', jobId)
      .is('archived_at', null)
  ...
  const fetchJobTasks = async (jobId: string) => {
    ...
    const primary = await supabase
      .from('job_tasks')
      .select('id, task_id, task_code, task_name, sequence_order, is_required, wait_after, estimated_minutes, custom_minutes, planned_minutes, notes, status, task:tasks!job_tasks_task_id_fkey(id, task_code, name, category, subcategory, priority_level, default_minutes, is_active)')
      .eq('job_id', jobId)
      .is('archived_at', null);
    ...
    const fallback = await supabase
      .from('job_tasks')
      .select('id, task_id, task_code, task_name, is_required, planned_minutes, notes, status, task:tasks!job_tasks_task_id_fkey(id, task_code, name, category, subcategory, priority_level, default_minutes, is_active)')
      .eq('job_id', jobId)
      .is('archived_at', null);
  ...
  const fetchJob = async () => {
    ...
    const { data } = await supabase
      .from('site_jobs')
      .select(
        '*, site:sites!site_jobs_site_id_fkey(site_code, name, client:clients!sites_client_id_fkey(name, client_code))'
      )
      .eq('job_code', id)
      .is('archived_at', null)
      .single();
