  ...
  const fetchDetails = useCallback(async () => {
    ...
    const [assignRes, staffRes, dayAssignRes, timeRes, exceptionsRes, inspectionsRes, issuesRes, suppliesRes, requirementsRes, checkoutsRes, tradesRes, currentStaffRes] = await Promise.all([
      ...
      supabase
        ...
        .select('*, staff:staff_id!ticket_assignments_staff_id_fkey(staff_code, full_name, role)')
      ...
      supabase
        .from('time_entries')
        .select('id, start_at, end_at, duration_minutes, status, staff:staff_id!time_entries_staff_id_fkey(full_name)')
        .eq('ticket_id', ticket.id)
        .is('archived_at', null)
      ...
      supabase
        ...
        .select('*, staff:staff_id!time_exceptions_staff_id_fkey(full_name, staff_code)')
        .is('archived_at', null)
        .order('created_at', { ascending: false }),
      // Inspections for this ticket
      supabase
        .from('inspections')
        .select('*, inspector:inspector_id!inspections_inspector_id_fkey(full_name)')
        .eq('ticket_id', ticket.id)
        .is('archived_at', null)
        .order('created_at', { ascending: false }),
      ...
      supabase
        ...
        .select('id, request_type, status, target_staff_id, initiator_staff_id, initiator_note, manager_note, requested_at, initiator:initiator_staff_id!shift_trade_requests_initiator_staff_id_fkey(full_name, staff_code), target:target_staff_id(full_name, staff_code)')
        .eq('ticket_id', ticket.id)
        .is('archived_at', null)
    ...
    const { data: usageData } = await supabase
      ...
      .select('*, supply:supply_id!ticket_supply_usage_supply_id_fkey(code, name)')
      .eq('ticket_id', ticket.id)
      .is('archived_at', null)
