...
import { Users, Target, FileText, Send, Plus, Zap, Calculator } from 'lucide-react';
...
import { ExpressBid } from '../pipeline/bids/express-bid';
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Prospects, Opportunities, Bids, Proposals</p>
        </div>
      ...
      {tab === 'prospects' && <ProspectsTable key={`prospects-${refreshKey}`} search={search} />}
      {tab === 'opportunities' && <OpportunitiesTable key={`opportunities-${refreshKey}`} search={search} />}
      {tab === 'bids' && <BidsTable key={`bids-${refreshKey}`} search={search} onCreateNew={() => setWizardOpen(true)} />}
      {tab === 'proposals' && <ProposalsTable key={`proposals-${refreshKey}`} search={search} onGoToBids={() => setTab('bids')} />}
      ...
      <ProspectForm
        ...
        onClose={() => { setProspectFormOpen(false); setEditProspect(null); }}
      ...
      <OpportunityForm
        ...
        onClose={() => { setOpportunityFormOpen(false); setEditOpportunity(null); }}
