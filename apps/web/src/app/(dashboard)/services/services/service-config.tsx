...
export default function ServiceConfig({ search, autoCreate, onAutoCreateHandled, onRefresh }: ServiceConfigProps) {
  ...
  const fetchLinkedTasks = useCallback(async (serviceId: string) => {
    ...
    const { data } = await supabase
      .from('service_tasks')
      .select('id, task_id, frequency_default, sequence_order, is_required, estimated_minutes, quality_weight, priority_level, task:task_id!service_tasks_task_id_fkey(task_code, name, category)')
