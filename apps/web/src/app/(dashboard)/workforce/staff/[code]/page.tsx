...
export default function StaffDetailPage() {
    ...
    if (data) {
      ...
      Promise.all([
        supabase
          ...
          .select('id, role, start_date, end_date, job:job_id(job_code, job_name, status, frequency, schedule_days, site:site_id(name, site_code, client:client_id!sites_client_id_fkey(name, client_code)))')
          .eq('staff_id', data.id)
          .is('archived_at', null)
  ...
  const loadAssignableJobs = async () => {
    ...
    const { data } = await supabase
      .from('site_jobs')
      .select('id, job_code, job_name, site:site_id(name, client:client_id!sites_client_id_fkey(name))')
      .eq('status', 'ACTIVE')
      .is('archived_at', null)
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-3">
            {(staff.photo_thumbnail_url || staff.photo_url) ? (
              <img src={staff.photo_thumbnail_url || staff.photo_url!} alt={staff.full_name} className="h-12 w-12 rounded-full object-cover" />
            ) : (
