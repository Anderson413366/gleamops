...
import { archiveAvailability } from '@/modules/schedule';
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await archiveAvailability(getUserClient(request), getServiceClient(), auth, request, id, API_PATH);
