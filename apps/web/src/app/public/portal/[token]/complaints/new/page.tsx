...
export default function PortalNewComplaintPage() {
  ...
  const [category, setCategory] = useState('CLEANING_QUALITY');
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const body = await response.json().catch(() => ({}));
      if (!response.ok) {
        throw new Error(body.error ?? 'Unable to submit complaint.');
      }
      toast.success('Complaint submitted.');
      router.push(`/public/portal/${encodeURIComponent(token)}/complaints`);
    } catch (error) {
  ...
  return (
    <Card className="rounded-xl border border-border bg-card shadow-sm">
      ...
      <CardContent className="space-y-4">
        <Link href={`/public/portal/${encodeURIComponent(token)}/complaints`} className="inline-flex items-center gap-2 text-sm text-muted-foreground hover:text-foreground">
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <Textarea
            ...
            rows={5}
            placeholder="Describe the issue clearly so operations can respond quickly."
          />
