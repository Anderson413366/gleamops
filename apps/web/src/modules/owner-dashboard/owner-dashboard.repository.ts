...
export async function listSupplyCostsForRange(
  ...
  let query = db
    ...
    .select(`
      ...
      archived_at,
      version_etag,
      site:sites!site_supply_costs_site_id_fkey(id, site_code, name),
      supply:supply_catalog!site_supply_costs_supply_id_fkey(id, code, name, category)
    `)
...
export async function listMicrofiberWashLogs(
  ...
  let query = db
    ...
    .select(`
      ...
      archived_at,
      version_etag,
      staff:staff!microfiber_wash_log_staff_id_fkey(id, staff_code, full_name, microfiber_rate_per_set),
      site:sites!microfiber_wash_log_site_id_fkey(id, site_code, name)
    `)
    .is('archived_at', null)
