...
export async function findRecentInspections(db: SupabaseClient, siteIds: string[]) {
  return db
    ...
    .select(`
      ...
      score_pct,
      summary,
      site:sites!inspections_site_id_fkey(id, name, site_code)
    `)
    .in('site_id', siteIds)
    .is('archived_at', null)
...
export async function findRecentCounts(db: SupabaseClient, siteIds: string[]) {
  return db
    ...
    .select(`
      ...
      counted_by_name,
      submitted_at,
      site:sites!inventory_counts_site_id_fkey(id, name, site_code)
    `)
    .in('site_id', siteIds)
    .is('archived_at', null)
...
export async function findRecentOrders(db: SupabaseClient, siteIds: string[]) {
  return db
    ...
    .select(`
      ...
      status,
      total_amount,
      site:sites!supply_orders_site_id_fkey(id, name, site_code)
    `)
    .in('site_id', siteIds)
    .is('archived_at', null)
...
export async function findCustomerPortalSessionByHash(
  ...
  return db
    ...
    .select(`
      ...
      archived_at,
      version_etag,
      client:clients!customer_portal_sessions_client_id_fkey(id, client_code, name)
    `)
...
export async function listPortalInspectionsBySites(
  ...
  return db
    ...
    .select(`
      ...
      summary_notes,
      photos,
      site:sites!inspections_site_id_fkey(id, site_code, name),
      inspector:staff!inspections_inspector_id_fkey(id, staff_code, full_name)
    `)
    .in('site_id', siteIds)
...
export async function insertCustomerFeedback(
  ...
  return db
    ...
    .select(`
      ...
      archived_at,
      version_etag,
      site:sites!customer_feedback_site_id_fkey(id, site_code, name)
    `)
    .single();
}
