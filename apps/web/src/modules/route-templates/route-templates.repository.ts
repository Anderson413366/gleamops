...
export async function listRouteTemplates(
  ...
  let query = db
    ...
    .select(`
      ...
      archived_at,
      version_etag,
      assigned_staff:assigned_staff_id!route_templates_assigned_staff_id_fkey(id, staff_code, full_name),
...
export async function getRouteStopSupplyContext(
  ...
  return db
    ...
    .select(`
      id,
      route_id,
      site_job:site_job_id!route_stops_site_job_id_fkey(site_id),
      route:route_id!route_stops_route_id_fkey(id, tenant_id, route_date)
    `)
    .eq('id', routeStopId)
    .is('archived_at', null)
    .maybeSingle();
