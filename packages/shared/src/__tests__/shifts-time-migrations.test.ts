...
describe('Shifts & Time migration guards', () => {
  it('enforces tenant WITH CHECK on UPDATE policies in 00106', () => {
    const sql = loadMigration('00106_shifts_time_rls_enforcement.sql');
  ...
  it('covers all planned Shifts & Time tables in 00106 enforcement list', () => {
    const sql = loadMigration('00106_shifts_time_rls_enforcement.sql');
  ...
  it('guards route execution RPCs to operations roles', () => {
    const sql = loadMigration('00107_shifts_time_functions.sql');
  ...
  it('keeps legacy route stop execution columns in sync in 00097', () => {
    const sql = loadMigration('00108_shifts_time_rpc_compat.sql');
    const startStopBlock = getFunctionBlock(sql, 'fn_route_start_stop');
    const completeStopBlock = getFunctionBlock(sql, 'fn_route_complete_stop');
